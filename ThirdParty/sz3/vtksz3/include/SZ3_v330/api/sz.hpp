#ifndef SZ3_V330_SZ_HPP
#define SZ3_V330_SZ_HPP

#include "SZ3_v330/api/impl/SZImpl.hpp"
#include "SZ3_v330/version.hpp"
...
SZ3_v330::Config conf(100, 200, 300); // 300 is the fastest dimension
conf.cmprAlgo = SZ3_v330::ALGO_INTERP_LORENZO;
conf.errorBoundMode = SZ3_v330::EB_ABS; // refer to def.hpp for all supported error bound mode
conf.absErrorBound = 1E-3; // absolute error bound 1e-3
char *compressedData = SZ_compress_v330(conf, data, outSize);

Interpolation example:
SZ3_v330::Config conf(100, 200, 300); // 300 is the fastest dimension
conf.cmprAlgo = SZ3_v330::ALGO_INTERP;
conf.errorBoundMode = SZ3_v330::EB_REL; // refer to def.hpp for all supported error bound mode
conf.relErrorBound = 1E-3; // value-rang-based error bound 1e-3
char *compressedData = SZ_compress_v330(conf, data, outSize);

Lorenzo/regression example :
SZ3_v330::Config conf(100, 200, 300); // 300 is the fastest dimension
conf.cmprAlgo = SZ3_v330::ALGO_LORENZO_REG;
 ...
 */
template <class T>
size_t SZ_compress_v330(const SZ3_v330::Config &config, const T *data, char *cmpData, size_t cmpCap) {
    using namespace SZ3_v330;
    Config conf(config);

    if (cmpCap < SZ_compress_size_bound_v330<T>(conf)) {
        throw std::invalid_argument(SZ3_V330_ERROR_COMP_BUFFER_NOT_LARGE_ENOUGH);
    }
 ...
 *
 * Similar with SZ_compress_v330(SZ3_v330::Config &conf, const T *data, char *cmpData, size_t cmpCap)
 * The only difference is this one doesn't need the pre-allocated buffer (thus remember to do 'delete []' yourself)
 */
template <class T>
char *SZ_compress_v330(const SZ3_v330::Config &config, const T *data, size_t &cmpSize) {
    using namespace SZ3_v330;

    size_t bufferLen = SZ_compress_size_bound_v330<T>(config);
    auto buffer = new char[bufferLen];
    cmpSize = SZ_compress_v330(config, data, buffer, bufferLen);
 ...
 * @param decData pre-allocated buffer for decompressed data
 ...
 SZ3_v330::Config conf;
 SZ_decompress_v330(conf, cmpData, cmpSize, decData);

 */
template <class T>
void SZ_decompress_v330(SZ3_v330::Config &config, const char *cmpData, size_t cmpSize, T *&decData) {
    using namespace SZ3_v330;
...
/**
 * API for decompression
 * Similar with SZ_decompress_v330(SZ3_v330::Config &config, char *cmpData, size_t cmpSize, T *&decData)
 ...
 * @return decompressed data, remember to 'delete []' when the data is no longer needed.
 ...
 example:
 SZ3_v330::Config conf;
 float decompressedData = SZ_decompress_v330(conf, cmpData, cmpSize)
 */
template <class T>
T *SZ_decompress_v330(SZ3_v330::Config &config, const char *cmpData, size_t cmpSize) {
    using namespace SZ3_v330;
    T *decData = nullptr;
    SZ_decompress_v330<T>(config, cmpData, cmpSize, decData);
    return decData;
}
