...
{
    using Entry = detailv3_v330::sherwood_v3_entry<T>;
...
private:
    ...
    int8_t max_lookups = detailv3_v330::min_lookups - 1;
    float _max_load_factor = 0.5f;
    size_t num_elements = 0;

    static int8_t compute_max_lookups(size_t num_buckets)
    {
        int8_t desired = detailv3_v330::log2(num_buckets);
        return std::max(detailv3_v330::min_lookups, desired);
    }
    ...
    void reset_to_empty_state()
    {
        ...
        hash_policy.reset();
        max_lookups = detailv3_v330::min_lookups - 1;
    }
...
struct power_of_two_hash_policy
{
    ...
    {
        size = detailv3_v330::next_power_of_two(size);
        return 0;
    }
    void commit(int8_t)
    {
...
struct fibonacci_hash_policy
{
    ...
    int8_t next_size_over(size_t & size) const
    {
        size = std::max(size_t(2), detailv3_v330::next_power_of_two(size));
        return 64 - detailv3_v330::log2(size);
    }
...
class flat_hash_map
        : public detailv3_v330::sherwood_v3_table
        <
            std::pair<K, V>,
            K,
            H,
            detailv3_v330::KeyOrValueHasher<K, std::pair<K, V>, H>,
            E,
            detailv3_v330::KeyOrValueEquality<K, std::pair<K, V>, E>,
            A,
            typename std::allocator_traits<A>::template rebind_alloc<detailv3_v330::sherwood_v3_entry<std::pair<K, V>>>
        >
{
    using Table = detailv3_v330::sherwood_v3_table
    <
        std::pair<K, V>,
        K,
        H,
...
class flat_hash_set
        ...
        <
            ...
            T,
            H,
            detailv3_v330::functor_storage<size_t, H>,
            E,
            detailv3_v330::functor_storage<bool, E>,
            A,
            typename std::allocator_traits<A>::template rebind_alloc<detailv3_v330::sherwood_v3_entry<T>>
        >
{
    using Table = detailv3_v330::sherwood_v3_table
    <
...
{
    typedef ska_v330::power_of_two_hash_policy hash_policy;
};
