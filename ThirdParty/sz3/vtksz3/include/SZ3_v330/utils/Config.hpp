...
#ifndef SZ3_V330_Config_HPP
#define SZ3_V330_Config_HPP
...
class Config {
   public:
    ...
    void load(const unsigned char *&c) {
        ...
        if (sz3MagicNumber != SZ3_V330_MAGIC_NUMBER) {
            throw std::invalid_argument("magic number mismatch, the input data is not compressed by SZ3");
        }
        read(sz3DataVer, c);
        if (versionStr_v330(sz3DataVer) != SZ3_V330_DATA_VER) {
            std::stringstream ss;
            printf("program v%s , program-data %s , input data v%s\n", SZ3_V330_VER, SZ3_V330_DATA_VER,
                   versionStr_v330(sz3DataVer).data());
            ss << "Please use SZ3 v" << versionStr_v330(sz3DataVer) << " to decompress the data" << std::endl;
            std::cerr << ss.str();
    ...
    uint32_t sz3MagicNumber = SZ3_V330_MAGIC_NUMBER;
    uint32_t sz3DataVer = versionInt_v330(SZ3_V330_DATA_VER);
    char N = 0;
