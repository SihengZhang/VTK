...
// Render 10 z-x slices of uncompressed Nek5000 data showing pressure field.
...
int RenderNek5000Uncompressed(int argc, char* argv[])
{
  ...
  // Check for uncompressed data file
  const char* dataFile = "/archives/disk1/viralss2/jicf_comp/uncjicf0.f00001";
  ...
  // Create output directory for uncompressed renders
  std::string outputDir = std::string(getenv("HOME")) + "/VTK/pressure_slices_uncompressed";
  if (myRank == 0)
  {
    mkdir(outputDir.c_str(), 0755);
    std::cout << "=== Rendering 10 Z-X Pressure Slices (Uncompressed) ===" << std::endl;
    std::cout << "MPI Ranks: " << numRanks << std::endl;
    std::cout << "Output directory: " << outputDir << std::endl;
  }

  // Create index file for first timestep only - uncompressed format
  const char* indexPath = "/tmp/render_pressure_slices_uncompressed.nek5000";
  if (myRank == 0)
  {
    std::ofstream nek5000File(indexPath);
    nek5000File << " filetemplate: /archives/disk1/viralss2/jicf_comp/uncjicf%01d.f%05d\n";
